...
public class Add_Customer extends JFrame implements  ActionListener{
    ...
    public Add_Customer() {
        ...
        setLocation(10, 10);
        setResizable(false);
        ImageIcon img = new ImageIcon(ClassLoader.getSystemResource("gym_management_system/cus1.jpg"));
    ...
    public void actionPerformed(ActionEvent e) {
        try {
            if (e.getSource() == bt3) {
                // Clear the choice component before adding new names
                ch5.removeAll();
            ...
            String idText = tf7.getText();
            String minag = ch2.getSelectedItem();
            String gender = ch3.getSelectedItem();
            String heightText = tf8.getText();
            String weightText = tf9.getText();
            String remarks = tf10.getText();

            if (e.getSource() == bt1) {
                // Check if any field is empty
                if (name.isEmpty() || fname.isEmpty() || email.isEmpty() || address.isEmpty() ||
                    cont.isEmpty() || tname.isEmpty() || idText.isEmpty() || heightText.isEmpty() ||
                    weightText.isEmpty() || remarks.isEmpty()) {
                ...
                try {
                    ...
                    if (id <= 0 || id > 999999) {
                        JOptionPane.showMessageDialog(null, "Receipt no. must be a positive integer less than 7 digits.", "Error", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                } catch (NumberFormatException ex) {
                    JOptionPane.showMessageDialog(null, "Please enter a valid numeric value for Receipt no.", "Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }

                // Validate Contact no.
                if (!cont.matches("\\d+")) {
                    JOptionPane.showMessageDialog(null, "Contact number must contain only numeric values.", "Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                ...
                try {
                    ...
                    if (height <= 0 || height > 999.99) {
                        JOptionPane.showMessageDialog(null, "Height must be a positive numeric value less than 1000.", "Error", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                } catch (NumberFormatException ex) {
                    JOptionPane.showMessageDialog(null, "Please enter a valid numeric value for Height.", "Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                ...
                try {
                    ...
                    if (weight <= 0 || weight > 999.99) {
                        JOptionPane.showMessageDialog(null, "Weight must be a positive numeric value less than 1000.", "Error", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                } catch (NumberFormatException ex) {
                    JOptionPane.showMessageDialog(null, "Please enter a valid numeric value for Weight.", "Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }
